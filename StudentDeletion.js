...
class DeleteStudent extends React.Component {
    ...
    handleDelete = (regNo) => {
        const url = `http://localhost:5000/deletestudent/${regNo}`; // Assuming this is your delete endpoint
        callApi("DELETE", url, null, this.handleDeleteSuccess, this.handleDeleteError);
    };

    handleDeleteSuccess = (response) => {
        alert("Student deleted successfully");
        // After successful deletion, you may want to refetch the list of students
        this.fetchStudents();
    };
                        ...
                        <tbody>
                            {filteredStudents.map((student, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{}</td>
                                    <td>
                                        <button onClick={() => this.handleDelete(student.regNo)} className="delete-button">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
